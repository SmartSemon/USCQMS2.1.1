...
public class SearchUtils
{
	...
	public static USCObject[] search(ItemInfo itemInfo, String queryWord, int page) throws Exception
	{
		if (!hasQueryFields(itemInfo))
		{
			return null;
		}
		USCObject[] dataList = null;
		if (!ObjectHelperUtils.isEmpty(queryWord))
		{
			String conString = "del=0 AND " + DBUtil.getItemQueryFieldsSql(itemInfo, queryWord);
			dataList = USCObjectQueryHelper.getObjectsByConditionLimit(itemInfo.getItemNo(), conString, page);
		} else
		{
			dataList = USCObjectQueryHelper.getObjectsByConditionLimit(itemInfo.getItemNo(), "del=0", page);
		}
		return dataList;
	...
	public static USCObject[] searchObjsByQueryCondition(ItemInfo itemInfo, String queryWord, String condition,
			int page) throws Exception
	{
		...
		USCObject[] objects = USCObjectQueryHelper.getObjectsByConditionLimit(itemInfo.getItemNo(), condition, page);
		return objects;
	}
