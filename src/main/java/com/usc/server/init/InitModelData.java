...
public class InitModelData {
	...
	private static final String MODELQUERYVIEWSQL = "SELECT * FROM usc_model_queryview WHERE del=0 AND (effective IN(-1,1) AND state='F') OR (effective=1 AND state='HS') ORDER BY CTIME";
	private static final String MODELCLASSVIEWSQL = "SELECT * FROM usc_model_classview WHERE del=0 AND (effective IN(-1,1) AND state='F') OR (effective=1 AND state='HS') ORDER BY CTIME";
	private static final String MODELMENULIBRARYSQL = "SELECT * FROM usc_model_menu WHERE del=0 AND (effective IN(-1,1) AND state='F') OR (effective=1 AND state='HS') ORDER BY CTIME";
	private static final String MODELGLOBALGRIDSQL = "SELECT * FROM usc_model_grid_global WHERE del=0 AND  state='F' ORDER BY CTIME";
	private static final String MODELGLOBALGRID = "MODELGLOBALGRID";
	...
	public static void initModel() {
		...
		List<GlobalGrid> globalGrids = jdbcTemplate.query(MODELGLOBALGRIDSQL, new GlobalGridRowMapper());
		if (!ObjectHelperUtils.isEmpty(globalGrids))
		{
			globalGrids.forEach(grid -> {
				redisUtil.hdel(MODELGLOBALGRID, grid.getNo());
				redisUtil.hset(MODELGLOBALGRID, grid.getNo(), grid);
			});
		}
	}
